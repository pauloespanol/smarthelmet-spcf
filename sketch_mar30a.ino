...
//Sim Device Cel Phone #: +63 905 231 5602
...
int SMS_CONNECTED_LED=19;
int SMS_NOT_CONNECTED_LED=18;

int GPS_CONNECTED_LED=2;
int GPS_NOT_CONNECTED_LED=15;
...
const float crashThreshold = 40.0;  // Adjust this value based on your requirements
//ADXL345 Settings  ----------------------
...
#define SMS_TARGET  "+639456690456"
#define CALL_TARGET "+639456690456"
...
void setup()
{
    ...
    pinMode(SMS_CONNECTED_LED,OUTPUT);
    pinMode(SMS_NOT_CONNECTED_LED,OUTPUT);
    pinMode(GPS_CONNECTED_LED,OUTPUT);
    pinMode(GPS_NOT_CONNECTED_LED,OUTPUT);
  ...
  // Optionally, set the range to increase sensitivity
    ...
    if (modem.isNetworkConnected()) {
      ...
      digitalWrite(SMS_CONNECTED_LED, HIGH);
      digitalWrite(SMS_NOT_CONNECTED_LED, HIGH);
      
    } else {
      SerialMon.println("Network Not Connected");
      digitalWrite(SMS_CONNECTED_LED, LOW);
      digitalWrite(SMS_NOT_CONNECTED_LED, HIGH);

    }
    digitalWrite(GPS_NOT_CONNECTED_LED, LOW);
...
while (Serial2.available() > 0) {
    //displayInfo();
      if (gps.encode(Serial2.read())) {
        if (gps.location.isValid()) {
          ...
          Serial.println("GPS signal Acquired...");
          GPS_Signal=true;
          digitalWrite(GPS_CONNECTED_LED, HIGH);
          digitalWrite(GPS_NOT_CONNECTED_LED, LOW);
          ...
          Serial.println("Waiting for GPS signal...");
          digitalWrite(GPS_NOT_CONNECTED_LED, HIGH);
          digitalWrite(GPS_CONNECTED_LED, LOW);
        }
      }
  }
  ...
  if (GPS_Signal==true) {
    ...
    if(btnState==1){
        digitalWrite(SMS_CONNECTED_LED,HIGH);
        digitalWrite(SMS_NOT_CONNECTED_LED,LOW);
        FalseAlarmAlert();
...
void FalseAlarmAlert() {
      String  FalseAlarm = "Message sent is a False Alert. I am ok and in good condition"  ;
        ...
        bool  locationsent = modem.sendSMS(SMS_TARGET, FalseAlarm);
  ...
  while (Serial2.available() > 0) {
    ...
    if (gps.encode(Serial2.read())) {
      if (gps.location.isValid()) {
        ...
        String staticMapsUrl = "Notice: Phil Telco Blocks All SMS with Link.\nCopy the Coordinate directly to your Google Map\n\nAlert: Crash Detected\n Rider Name:Rizal Jose \n Blood type: O \n Address Manila \n Emergency Contact Name : Ramon Magsaysay \n Address: Manila \n Contact No. 09923008195 \n\nPLEASE SEND AN EMERGENCY RESCUE TEAM IMMEDIATELY!!!\nGPS Location at: "  ;
